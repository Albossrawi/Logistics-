...
const SYSTEM = `You read shipping/delivery labels (often DSV labels) from photos and extract exactly two fields:
...
const PROMPT = `Extract the delivery number and reference number from this label.
Respond with ONLY a compact JSON object and nothing else, e.g.:
{"deliveryNumber":"NAKD1-8FL64","referenceNumber":"SRV010001"}`;
...
function parseJson(text: string): { deliveryNumber?: string; referenceNumber?: string } {
